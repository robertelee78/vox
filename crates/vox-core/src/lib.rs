...
/// Helpers for this crate's own unit tests. Not part of the public API.
#[cfg(test)]
pub(crate) mod test_support {
    /// Whether the named type is `Clone`, decided by *autoref specialization*
    /// on stable Rust: for a receiver `&Probe<T>`, method resolution tries the
    /// by-reference impl on `Probe<T>` (present only when `T: Clone`) before
    /// autoref-ing to the fallback impl on `&Probe<T>`.
    ///
    /// This must be a macro, not a generic `fn is_clone<T>()`: inside a generic
    /// function `T: Clone` is unprovable at type-check time, so the `Clone`
    /// branch would never be selected and the check would be vacuous (which is
    /// exactly the defect it replaces). Every use pairs the negative assertion
    /// with a positive control such as `Vec<u8>`.
    macro_rules! is_clone {
        ($t:ty) => {{
            struct Probe<T>(core::marker::PhantomData<T>);
            // Exactly one of the two traits is selected per expansion; the
            // other is dead by design.
            #[allow(dead_code)]
            trait IfClone {
            ...
            impl<T: Clone> IfClone for Probe<T> {}
            #[allow(dead_code)]
            trait IfNotClone {
                fn is_clone(&self) -> bool {
                    false
            ...
            impl<T> IfNotClone for &Probe<T> {}
            (&Probe::<$t>(core::marker::PhantomData)).is_clone()
        }};
    }
    pub(crate) use is_clone;
}
