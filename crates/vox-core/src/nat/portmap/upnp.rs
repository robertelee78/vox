...
#[cfg(test)]
pub(crate) mod mock {
    //! An in-process Internet Gateway Device that follows the specifications, with
    //! the quirks real routers are known for switchable, so the client can be proved
    //! on a network with no router to prove it against.
    use super::*;
    ...
    pub struct Quirks {
        /// Refuse timed leases with 725, as some routers do.
        pub permanent_only: bool,
        /// Declare only a `WANPPPConnection` service.
        pub ppp_only: bool,
        /// Put a `<URLBase>` in the description and make the control URL relative.
        pub url_base: bool,
        /// Serve the description with `Transfer-Encoding: chunked`.
        pub chunked: bool,
        /// Point `LOCATION` at a host that is not the responder.
        pub location_elsewhere: bool,
        /// Answer SSDP but serve no WAN service at all.
        pub no_wan_service: bool,
    }

    /// What the mock saw, for assertions.
    #[derive(Clone, Debug, Default)]
    pub struct Seen {
        /// Every `(action, arguments)` SOAP request, in order.
        pub actions: Vec<(String, String)>,
    }

    /// A running mock gateway.
    pub struct MockIgd {
        /// Where the client should send its M-SEARCH (unicast, in tests).
    ...
    impl MockIgd {
        pub async fn start(quirks: Quirks) -> MockIgd {
            ...
            // SSDP: answer every M-SEARCH with a 200 OK naming the description.
            let location_host = if quirks.location_elsewhere {
                "http://203.0.113.200:80".to_owned()
            } else {
                ...
                while let Ok((n, from)) = ssdp.recv_from(&mut buf).await {
                    ...
                    let reply = format!(
                        "HTTP/1.1 200 OK\r\nCACHE-CONTROL: max-age=120\r\nEXT:\r\n\
                         LOCATION: {location_host}/rootDesc.xml\r\nSERVER: MockOS/1.0 UPnP/1.1 mock/1\r\n\
                         ST: {SEARCH_TARGET}\r\nUSN: uuid:0000-mock::{SEARCH_TARGET}\r\n\r\n"
                    );
                    let _ = ssdp.send_to(reply.as_bytes(), from).await;
                }
            });

            // HTTP: the description and the control endpoint.
            let q = quirks.clone();
            let seen_http = Arc::clone(&seen);
            tokio::spawn(async move {
                while let Ok((mut stream, _)) = listener.accept().await {
                    let q = q.clone();
                    let seen = Arc::clone(&seen_http);
                    tokio::spawn(async move {
                        let mut raw = Vec::new();
                        let mut chunk = [0u8; 4096];
                        ...
                        let head = String::from_utf8_lossy(&raw[..split]).to_string();
                        let body =
                            String::from_utf8_lossy(&raw[split.min(raw.len())..]).to_string();
                        let request_line = head.lines().next().unwrap_or("").to_owned();
                        let response = if request_line.starts_with("GET /rootDesc.xml") {
                            description(&q, http_addr)
                        } else if request_line.starts_with("POST /ctl/IPConn")
                            || request_line.starts_with("POST /ctl/PPPConn")
                        {
                            control(&q, &head, &body, &seen, external_ip)
                        } else {
    ...
    fn description(q: &Quirks, http_addr: SocketAddr) -> String {
        let (st, url) = if q.ppp_only {
            (
        ...
        let url = if q.url_base {
            url.trim_start_matches('/').to_owned()
        } else {
            url.to_owned()
        };
        let url_base = if q.url_base {
            format!("<URLBase>http://{http_addr}/</URLBase>")
        } else {
            String::new()
        };
        let services = if q.no_wan_service {
            ...
            format!(
                "<service><serviceType>urn:schemas-upnp-org:service:Layer3Forwarding:1</serviceType>\
                 <controlURL>/ctl/L3F</controlURL></service>\
                 <service><serviceType>{st}</serviceType><serviceId>urn:upnp-org:serviceId:WANIPConn1</serviceId>\
                 <controlURL>{url}</controlURL><eventSubURL>/evt/IPConn</eventSubURL></service>"
            )
        };
        let xml = format!(
            "<?xml version=\"1.0\"?><root xmlns=\"urn:schemas-upnp-org:device-1-0\">\
             <specVersion><major>1</major><minor>0</minor></specVersion>{url_base}\
             <device><deviceType>urn:schemas-upnp-org:device:InternetGatewayDevice:1</deviceType>\
             <friendlyName>Mock Router</friendlyName><deviceList><device>\
             <deviceType>urn:schemas-upnp-org:device:WANDevice:1</deviceType><deviceList><device>\
             <deviceType>urn:schemas-upnp-org:device:WANConnectionDevice:1</deviceType>\
             <serviceList>{services}</serviceList></device></deviceList></device></deviceList>\
             </device></root>"
        );
        if q.chunked {
            // Two chunks, to prove the decoder joins them.
            let (a, b) = xml.split_at(xml.len() / 2);
            format!(
    ...
    fn control(
        ...
        let action = head
            ...
            .find_map(|l| {
                l.split_once(':')
                    .filter(|(k, _)| k.eq_ignore_ascii_case("soapaction"))
                    .map(|(_, v)| v.trim().trim_matches('"').to_owned())
            })
            .and_then(|a| a.rsplit_once('#').map(|(_, a)| a.to_owned()))
            .unwrap_or_default();
        let st = if q.ppp_only {
            "urn:schemas-upnp-org:service:WANPPPConnection:1"
        } else {
            "urn:schemas-upnp-org:service:WANIPConnection:1"
        };
        seen.lock()
            .unwrap()
            .actions
            .push((action.clone(), body.to_owned()));
        let ok = |inner: &str| {
            let xml = format!(
                "<?xml version=\"1.0\"?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" \
                 s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><s:Body>\
                 <u:{action}Response xmlns:u=\"{st}\">{inner}</u:{action}Response></s:Body></s:Envelope>"
            );
        ...
        match action.as_str() {
            "AddPortMapping" => {
                let lease: u32 = element(body, "NewLeaseDuration")
                    .and_then(|l| l.trim().parse().ok())
                    .unwrap_or(0);
                if q.permanent_only && lease != 0 {
                    fail(ONLY_PERMANENT_LEASES)
                } else {
                    ok("")
                }
            }
            "DeletePortMapping" => ok(""),
            "GetExternalIPAddress" => ok(&format!(
...
#[cfg(test)]
mod tests {
    use super::mock::{MockIgd, Quirks};
    use super::*;

    async fn discover_mock(m: &MockIgd) -> Result<IgdGateway> {
        discover(Ipv4Addr::LOCALHOST, m.ssdp_addr, Duration::from_millis(800)).await
    }

    #[tokio::test]
    async fn discovery_maps_and_reads_the_external_address() {
        let m = MockIgd::start(Quirks::default()).await;
        let gw = discover_mock(&m).await.expect("the mock gateway is found");
        assert_eq!(gw.router, IpAddr::V4(Ipv4Addr::LOCALHOST));
        assert_eq!(gw.control_host, m.http_addr);
        assert_eq!(gw.control_path, "/ctl/IPConn");
        assert_eq!(
            gw.service_type,
            "urn:schemas-upnp-org:service:WANIPConnection:1"
        );

        let lease = add_port_mapping(
            &gw,
            Protocol::Udp,
            4433,
            4433,
        ...
        assert_eq!(get_external_ip(&gw).await.unwrap(), m.external_ip);
        delete_port_mapping(&gw, Protocol::Udp, 4433).await.unwrap();

        // The requests said what the specification says they must.
        let seen = m.seen.lock().unwrap().clone();
        assert_eq!(seen.actions.len(), 3);
        let (action, body) = &seen.actions[0];
        assert_eq!(action, "AddPortMapping");
        for must in [
            "<NewRemoteHost></NewRemoteHost>",
            "<NewExternalPort>4433</NewExternalPort>",
            "<NewProtocol>UDP</NewProtocol>",
            "<NewInternalPort>4433</NewInternalPort>",
            "<NewInternalClient>192.168.1.50</NewInternalClient>",
            "<NewEnabled>1</NewEnabled>",
            "<NewPortMappingDescription>Vox</NewPortMappingDescription>",
            "<NewLeaseDuration>7200</NewLeaseDuration>",
            "xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\"",
        ] {
            assert!(body.contains(must), "missing {must} in {body}");
        }
        assert_eq!(seen.actions[1].0, "GetExternalIPAddress");
        assert_eq!(seen.actions[2].0, "DeletePortMapping");
        assert!(seen.actions[2]
            .1
            .contains("<NewExternalPort>4433</NewExternalPort>"));
    }

    #[tokio::test]
    async fn a_router_that_only_grants_permanent_leases_gets_asked_again_with_zero() {
        let m = MockIgd::start(Quirks {
            permanent_only: true,
            ..Quirks::default()
        })
        .await;
        let gw = discover_mock(&m).await.unwrap();
        let lease = add_port_mapping(
            &gw,
            Protocol::Udp,
            4433,
        ...
        .await
        .unwrap();
        assert_eq!(lease, 0, "permanent: the caller must delete it itself");
        let seen = m.seen.lock().unwrap().clone();
        assert_eq!(seen.actions.len(), 2);
        assert!(seen.actions[0]
            .1
            .contains("<NewLeaseDuration>7200</NewLeaseDuration>"));
        assert!(seen.actions[1]
            .1
            .contains("<NewLeaseDuration>0</NewLeaseDuration>"));
    }

    #[tokio::test]
    async fn real_router_quirks_are_handled() {
        // PPPoE router: only a WANPPPConnection service, under a URLBase with a
        // relative control URL, description served chunked.
        let m = MockIgd::start(Quirks {
            ppp_only: true,
            url_base: true,
            chunked: true,
        ...
        .await;
        let gw = discover_mock(&m)
            .await
            .expect("found through every quirk at once");
        assert_eq!(
            gw.service_type,
            "urn:schemas-upnp-org:service:WANPPPConnection:1"
        );
        assert_eq!(gw.control_path, "/ctl/PPPConn");
        assert_eq!(gw.control_host, m.http_addr);
        assert_eq!(get_external_ip(&gw).await.unwrap(), m.external_ip);
    }

    #[tokio::test]
    async fn a_location_off_the_responder_is_never_followed() {
        let m = MockIgd::start(Quirks {
            location_elsewhere: true,
            ..Quirks::default()
        })
        .await;
        let err = discover_mock(&m).await.unwrap_err();
        assert!(
            matches!(
                err,
                Error::PortMappingFailed("upnp: location is not on the responder")
            ),
            "{err:?}"
        );
    }
    ...
    #[tokio::test]
    async fn a_gateway_with_no_wan_service_is_not_a_gateway() {
        let m = MockIgd::start(Quirks {
            no_wan_service: true,
            ..Quirks::default()
        })
        ...
        assert!(
            matches!(
                err,
                Error::PortMappingFailed("upnp: no WAN connection service")
            ),
            "{err:?}"
        );
    }
    ...
    #[tokio::test]
    async fn silence_is_a_bounded_failure() {
        // Nothing listens here; the search must give up at its timeout.
        let dead = UdpSocket::bind((Ipv4Addr::LOCALHOST, 0)).await.unwrap();
        let addr = dead.local_addr().unwrap();
        drop(dead);
        let started = std::time::Instant::now();
        let err = discover(Ipv4Addr::LOCALHOST, addr, Duration::from_millis(300))
            .await
            .unwrap_err();
        assert!(matches!(
            err,
            Error::PortMappingFailed("upnp: no gateway answered")
        ));
        assert!(started.elapsed() < Duration::from_secs(2));
    }

    #[test]
    fn scanning_and_parsing_are_strict() {
        assert_eq!(element("<a><b>x</b></a>", "b"), Some("x"));
        assert_eq!(element("<u:b>y</u:b>", "b"), Some("y"));
        assert_eq!(element("<a>no b</a>", "b"), None);
        assert_eq!(
            parse_http_url("http://192.168.1.1:5000/desc.xml").unwrap(),
            ("192.168.1.1:5000".parse().unwrap(), "/desc.xml".to_owned())
        );
        assert_eq!(parse_http_url("http://192.168.1.1").unwrap().1, "/");
        assert!(parse_http_url("https://192.168.1.1/x").is_err());
        assert!(
            parse_http_url("http://router.local/x").is_err(),
            "no hostnames"
        );
        assert!(parse_http_url("http://[fe80::1]/x").is_err());
        assert_eq!(
            dechunk(b"3\r\nabc\r\n2\r\nde\r\n0\r\n\r\n").unwrap(),
            b"abcde"
        );
        ...
        assert!(ssdp_location(b"NOTIFY * HTTP/1.1\r\nLOCATION: http://x/y\r\n\r\n").is_none());
        // WANIPConnection is preferred over WANPPPConnection when both are present.
        let both = "<service><serviceType>urn:schemas-upnp-org:service:WANPPPConnection:1</serviceType><controlURL>/p</controlURL></service>\
                    <service><serviceType>urn:schemas-upnp-org:service:WANIPConnection:2</serviceType><controlURL>/i</controlURL></service>";
        assert_eq!(wan_connection_service(both).unwrap().1, "/i");
    }
}
