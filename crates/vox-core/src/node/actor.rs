...
impl Node {
    ...
    async fn release_key_to(
        ...
        // session-opening `Hello`: the rest ride the session it established.
        for (i, one) in skdm.iter().enumerate() {
            let opening = if i == 0 { hello.as_ref() } else { None };
            if let Err(e) =
                crate::node::pairwise_stream::deliver_skdm(&conn, channel_id, session, one, opening)
                    .await
            {
                return Outcome::Failed(fault_of(&e));
            }
        }
        let Some(skdm) = skdm.last() else {
        ...
        if let Err(e) = shared
            .lock()
            .await
            .issue_consent(profile, target, skdm, full, now)
        {
            return Outcome::Failed(fault_of(&e));
        }
        Outcome::Done
    ...
    async fn consent(&mut self, channel_id: &Digest32, target: Digest32) -> Outcome {
        ...
        let outcome = self.release_key_to(channel_id, target, history).await;
        if outcome.is_done() {
