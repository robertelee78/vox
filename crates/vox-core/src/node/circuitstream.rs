...
mod tests {
    ...
    #[test]
    fn every_frame_round_trips_and_decoding_is_strict() {
        let frames = [
            CircuitFrame::Open { peer: [1u8; 32] },
            CircuitFrame::Incoming { peer: [2u8; 32] },
            CircuitFrame::Opened,
            CircuitFrame::Refused {
                reason: CircuitRefusal::NotConnected,
            },
            CircuitFrame::Refused {
                reason: CircuitRefusal::NotAuthorized,
            },
            CircuitFrame::Refused {
                reason: CircuitRefusal::Capacity,
            },
            CircuitFrame::Datagram {
                payload: vec![0xC0; 1200],
            },
        ];
        for frame in frames {
            let bytes = frame.to_bytes();
            assert!(bytes.len() <= MAX_CIRCUIT_FRAME);
            assert_eq!(CircuitFrame::from_bytes(&bytes).unwrap(), frame);
        }
        let mut e = Encoder::new();
        e.array(1).uint(77);
        assert!(CircuitFrame::from_bytes(&e.finish()).is_err());
        let mut e = Encoder::new();
        e.array(2).uint(OP_OPENED).uint(0);
        assert!(CircuitFrame::from_bytes(&e.finish()).is_err());
        let mut e = Encoder::new();
        e.array(2).uint(OP_OPEN).bytes(&[1u8; 31]);
        assert!(CircuitFrame::from_bytes(&e.finish()).is_err());
        let mut e = Encoder::new();
        e.array(2).uint(OP_REFUSED).uint(9);
        assert!(CircuitFrame::from_bytes(&e.finish()).is_err());
        let mut bytes = CircuitFrame::Opened.to_bytes();
        bytes.push(0);
        assert!(CircuitFrame::from_bytes(&bytes).is_err());
    }

    #[test]
    fn the_ledger_enforces_both_caps_and_gives_places_back() {
        let ledger = Arc::new(CircuitLedger::default());
        let asker = [7u8; 32];
        let mut held = Vec::new();
        for _ in 0..MAX_CIRCUITS_PER_ASKER {
            held.push(ledger.take(asker).expect("under the per-asker cap"));
        }
        assert!(
            ledger.take(asker).is_none(),
            "one asker cannot take a fifth"
        );
        assert_eq!(ledger.carrying(), MAX_CIRCUITS_PER_ASKER);
        // Other askers still can, up to the total.
        let mut others = Vec::new();
        let mut seed = 8u8;
        while ledger.carrying() < MAX_RELAYED_CIRCUITS {
            others.push(ledger.take([seed; 32]).expect("under the total"));
            seed = seed.wrapping_add(1);
        }
        assert!(ledger.take([250u8; 32]).is_none(), "the total cap holds");
        // Places come back on drop.
        drop(held);
        assert_eq!(
            ledger.carrying(),
            MAX_RELAYED_CIRCUITS - MAX_CIRCUITS_PER_ASKER
        );
        assert!(ledger.take(asker).is_some());
        drop(others);
    }
}
