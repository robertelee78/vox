...
///
/// `asker_conn` is the connection the stream arrived on, which its datagram flow is
/// bound to; `connected` resolves a fingerprint to a live connection (the relay's own
/// peer table); `endpoint` is where a circuit terminating here is attached.
#[allow(clippy::too_many_arguments)]
pub async fn serve_circuit<F>(
    peer: Digest32,
    asker_conn: &VoxConnection,
    ...
    F: FnOnce(&Digest32) -> Option<Arc<VoxConnection>>,
{
    match opening_answer(&mut recv).await? {
        CircuitFrame::Open { peer: target } => {
            ...
            let asker_flow = asker_conn.bind_forwarding_flow(send, recv)?;
            tokio::spawn(relay(slot, asker_flow, target_flow));
            Ok(())
        }
...
pub async fn connect_through(
    ...
    let driver = DriverGuard::new(tokio::spawn(terminate(port, flow)));
    let conn =
...
/// ledger is given back then too.
async fn relay(slot: CircuitSlot, mut asker: DatagramFlow, mut target: DatagramFlow) {
