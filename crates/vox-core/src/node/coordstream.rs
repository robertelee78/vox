...
mod tests {
    ...
    fn ip4(host: u8, port: u16) -> Multiaddr {
        Multiaddr::Ip4(SocketAddrV4::new(Ipv4Addr::new(10, 0, 0, host), port))
    }

    fn frames() -> Vec<CoordFrame> {
        vec![
            CoordFrame::WhoAmI,
            CoordFrame::Observed { addr: ip4(7, 4433) },
            CoordFrame::Relay { peer: [9u8; 32] },
            CoordFrame::From { peer: [3u8; 32] },
            CoordFrame::Relaying,
            CoordFrame::Refused {
                reason: CoordRefusal::NotConnected,
            },
            CoordFrame::Refused {
                reason: CoordRefusal::NotAuthorized,
            },
            CoordFrame::Refused {
                reason: CoordRefusal::Capacity,
            },
            CoordFrame::Coord {
                payload: CoordMessage::Sync.to_bytes(),
            },
        ]
    }
    ...
    fn every_frame_round_trips() {
        for frame in frames() {
            ...
            assert!(bytes.len() <= MAX_COORD_FRAME);
            assert_eq!(
                CoordFrame::from_bytes(&bytes).unwrap(),
                frame,
    ...
    fn decoding_is_strict() {
        ...
        let mut e = Encoder::new();
        e.array(1).uint(99);
        assert!(CoordFrame::from_bytes(&e.finish()).is_err());
        // A known op with the wrong arity.
        let mut e = Encoder::new();
        e.array(2).uint(OP_WHOAMI).uint(0);
        assert!(CoordFrame::from_bytes(&e.finish()).is_err());
        // A fingerprint that is not 32 bytes.
        let mut e = Encoder::new();
        e.array(2).uint(OP_RELAY).bytes(&[1u8; 16]);
        assert!(CoordFrame::from_bytes(&e.finish()).is_err());
        // An unknown refusal reason.
        let mut e = Encoder::new();
        e.array(2).uint(OP_REFUSED).uint(42);
        assert!(CoordFrame::from_bytes(&e.finish()).is_err());
        // Trailing bytes after a complete frame.
        let mut bytes = CoordFrame::Relaying.to_bytes();
        bytes.push(0);
        assert!(CoordFrame::from_bytes(&bytes).is_err());
        // A truncated frame.
        let full = CoordFrame::Observed { addr: ip4(1, 1) }.to_bytes();
        assert!(CoordFrame::from_bytes(&full[..full.len() - 1]).is_err());
    }

    #[test]
    fn punch_endpoints_lead_with_what_a_coordinator_observed() {
        let local = EndpointList::new(vec![ip4(5, 4433), ip4(6, 4433)]).unwrap();
        let observed = ip4(200, 51820);
        let list = punch_endpoints(Some(observed), &local);
        assert_eq!(list.addrs()[0], observed, "the mapped address is dialable");
        assert_eq!(list.addrs().len(), 3);
        // An observed address this node already advertises is not repeated: a dialer
        // would waste an attempt on the duplicate.
        let same = punch_endpoints(Some(ip4(5, 4433)), &local);
        assert_eq!(same.addrs(), local.addrs());
        // With nothing observed, the local set is offered as it is — a peer on the same
        // LAN can still use it.
        assert_eq!(punch_endpoints(None, &local).addrs(), local.addrs());
    }

    #[test]
    fn the_offer_never_exceeds_the_endpoint_cap() {
        let local =
            EndpointList::new((0..MAX_ENDPOINTS as u8).map(|i| ip4(i, 4433)).collect()).unwrap();
        let list = punch_endpoints(Some(ip4(250, 51820)), &local);
        assert_eq!(list.addrs().len(), MAX_ENDPOINTS);
        assert_eq!(list.addrs()[0], ip4(250, 51820), "the observed one is kept");
    }

    #[test]
    fn only_peers_this_node_knows_are_relayed_for() {
        assert!(relays_for(PeerClass::Member));
        assert!(relays_for(PeerClass::Anchor));
        // A joiner has a pre-join record on this board: it is not anonymous, and a
        // swarm whose members are all behind NAT could not be joined otherwise.
