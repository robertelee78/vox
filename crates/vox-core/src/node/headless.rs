...
mod tests {
    ...
    use crate::node::actor::{Node, NodeConfig};
    use crate::node::api::{Fault, NodeCommand, Outcome};

    /// A headless node has nothing to lock, so `Lock` is refused: obeying it would
    /// take the anchor off the network with no passphrase to bring it back
    /// (ADR-016 M15.2c).
    #[tokio::test]
    async fn lock_is_refused_on_a_headless_node() {
        let tmp = tempfile::tempdir().unwrap();
        let paths =
            Paths::resolve("node", Some(tmp.path()), Some(&tmp.path().join("cfg"))).unwrap();
        let signer = load_or_create_identity(&paths).unwrap();
        let node = Node::spawn_config(
            paths,
            NodeConfig::new()
                .bind(crate::node::actor::Bind::Addr(
        ...
        assert!(!listening.is_empty(), "serving from spawn");
        assert_eq!(
            node.apply(NodeCommand::Lock).await,
            Outcome::Failed(Fault::NoIdentity)
        );
        assert_eq!(
            node.view().listening,
            listening,
            "still serving: a refused lock changed nothing"
        );
