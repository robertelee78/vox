...
mod tests {
    ...
    fn assert_sk_agrees(with_otp: bool) {
        ...
        let init = initiate(&ik_a, &b, &cid, 7, 0x0001, SuiteFloor::DAY_ONE).unwrap();
        let prekeys = ResponderPrekeys {
            identity_dh_key: &bob_idk,
            signed_prekey: &r.spk,
            one_time_prekey: otp.as_ref(),
        };
        let resp = accept(&init.message, &prekeys, &cid, 7, SuiteFloor::DAY_ONE).unwrap();
    ...
    fn initiate_rejects_unauthenticated_bundle() {
        ...
        let ik_a = X25519IdentityKey::generate().unwrap();
        assert!(matches!(
            initiate(&ik_a, &b, &[0u8; 32], 0, 0x0001, SuiteFloor::DAY_ONE),
    ...
    fn initial_message_wire_round_trips() {
        ...
        let ik_a = X25519IdentityKey::generate().unwrap();
        let init = initiate(&ik_a, &b, &[5u8; 32], 3, 0x0001, SuiteFloor::DAY_ONE).unwrap();
        ...
        let init2 = initiate(&ik_a, &b2, &[5u8; 32], 3, 0x0001, SuiteFloor::DAY_ONE).unwrap();
