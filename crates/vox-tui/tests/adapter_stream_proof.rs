...
fn start(w: &Worker, r: &str, cursor: &str, stderr: &std::path::Path) -> Run {
    ...
    cmd.args(["room", "tail", r, "--since", cursor, "--json"])
        ...
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(std::fs::OpenOptions::new().create(true).append(true).open(stderr).unwrap());
...
fn consume(run: &Run, n: usize, idle: Duration, cursor: &mut String, seen: &mut BTreeMap<String, u32>) -> usize {
    let mut got = 0;
    while got < n {
        let Ok(line) = run.rx.recv_timeout(idle) else { break };
        let row: serde_json::Value = serde_json::from_str(line.trim()).expect("NDJSON row");
        assert_eq!(row["schema"], "vox.room.row/1", "an adapter must refuse any other schema");
...
fn a_consumer_that_lags_and_crashes_three_times_misses_nothing() {
    ...
    assert!(alice.vox(Some("a1"), &["room", "claim", &r, "contested"]).ok);
    until(bob, None, "the claim to reach bob", &["room", "board", &r, "--json"], |o: &Out| {
        o.ok && support::resource(&o.json(), "contested").is_some()
    });
    assert_eq!(bob.vox(Some("b1"), &["room", "claim", &r, "contested"]).code, Some(1));
    assert!(alice.vox(Some("a1"), &["room", "claim", &r, "passed"]).ok);
    assert!(alice.vox(Some("a1"), &["room", "handoff", &r, "passed", "--to", &bob.b32()[..16]]).ok);
    until(bob, None, "the handoff to reach bob", &["room", "board", &r, "--json"], |o: &Out| {
        o.ok && support::resource(&o.json(), "passed").is_some_and(|x| x["state"] == "pending")
    });
    assert!(bob.vox(Some("b1"), &["room", "claim", &r, "passed"]).ok);
    assert!(bob.vox(Some("b1"), &["room", "claim", &r, "lapsed", "--ttl", "1"]).ok);
    std::thread::sleep(Duration::from_secs(2));
    until(bob, None, "bob to hold everything alice posted", &["room", "read", &r, "--json"], |o: &Out| {
        o.ok && o.ndjson().len() == alice.vox(None, &["room", "read", &r, "--json"]).ndjson().len()
    });
    ...
    let from_board: BTreeMap<String, (String, String)> = board["resources"]
        ...
        .unwrap()
        .iter()
        .map(|x| (x["resource"].as_str().unwrap().to_owned(), (x["owner_fp"].as_str().unwrap_or("").to_owned(), x["owner_session"].as_str().unwrap_or("").to_owned())))
        .collect();
    let from_fold: BTreeMap<String, (String, String)> = independent
        .resources
        .iter()
        .map(|(k, s)| match s {
            vox_agentcomms::State::Held { owner, .. } => (k.clone(), (vox_agentcomms::claim::b32(&owner.author), owner.session.clone())),
            vox_agentcomms::State::Pending { .. } => (k.clone(), (String::new(), String::new())),
        })
        .collect();
    assert_eq!(from_board, from_fold, "board --json is not the fold of the room's rows");
    assert_eq!(from_board.get("contested").map(|x| x.1.as_str()), Some("a1"));
    ...
    // ---- the stream half ----
    let start_cursor = rows.last().unwrap()["entry_hash"].as_str().unwrap().to_owned();
    ...
    let mut burst = |w: &Worker, count: usize| {
        ...
        rt.block_on(async move {
            ...
            for i in first..first + count {
                match c.request(&vox_core::node::ipc::Request::Post { channel_id: cid, text: say(i) }).await {
    ...
    let kill = |run: &mut Run, restarts: &mut u32, cursor: &str, seen: &BTreeMap<String, u32>| {
        ...
        *restarts += 1;
        eprintln!("[proof] killed consumer run {restarts} at cursor {cursor} after {} distinct rows", seen.len());
    };
    ...
    assert_eq!(live, 200, "rows synced from another node must reach a LIVE consumer, not only a restarted one");
    kill(&mut run, &mut restarts, &cursor, &seen);
    ...
    let all = until(bob, None, "bob to hold all 1,800 messages", &["room", "read", &r, "--json"], |o: &Out| {
        o.ok && o.ndjson().iter().filter(|x| x["text"].as_str().is_some_and(|t| t.starts_with("burst message"))).count() == 1800
    })
    .ndjson();
    let after: Vec<String> = all
        ...
        .skip(1)
        .collect();
    assert_eq!(after.len(), 1800, "the log after the starting cursor is the burst");
    ...
    let missing: Vec<&&String> = expected.iter().filter(|h| !seen.contains_key(**h)).collect();
    let extra: Vec<&String> = seen.keys().filter(|h| !expected.contains(h)).collect();
    let dups = seen.values().filter(|c| **c > 1).count();
    let lags = std::fs::read_to_string(&stderr).unwrap_or_default().matches("fell behind").count();
    eprintln!(
    ...
    assert!(missing.is_empty(), "GAP: {} rows after the cursor were never emitted: {:?}", missing.len(), &missing[..missing.len().min(5)]);
    assert!(extra.is_empty(), "rows from before the starting cursor were emitted: {extra:?}");
    assert!(lags > 0, "the consumer never lagged, so this run proved nothing about lag");
    assert!(dups as u32 <= restarts * 600, "duplicates beyond what restarts explain: {dups}");
}
