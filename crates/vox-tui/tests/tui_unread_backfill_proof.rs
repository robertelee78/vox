//! **PARKED until PRD-001 R12's "full history" approval mode exists — red today by
//! construction, not by defect.** Consent releases the sender chain at its current
//! position (`ChannelState::skdm_for_consent`), i.e. "from approval onward", so a message
//! posted before consent never renders and the backfill step below never happens between
//! two reachable nodes: this proof times out waiting for it. When a full-history grant
//! exists, alice's grant to bob must use it, and this becomes the gate for ADR-023 M23.4.
//!
...
//!
//! Mutation: dropping the `backfilled` count turns it red with the badge at 0 — which also
//! shows the row really took the key path, since a row rendered by a sync would be counted
//! by `Synced` with or without this fix.
