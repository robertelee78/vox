...
//! (PRD-001 R22, R23, R24; defects D7 and D11, and D6's `vox forward` half).
//!
...
//!
//! 1. **A forward survives its host restarting** (R24, D7). `vox forward` used to pin the
//!    one QUIC connection it started with, so once the host restarted every later
//!    connection was opened on a connection that went nowhere and the forward was dead while
//!    still bound. Here the host's `vox serve` is killed and the same room brought back by
//!    `vox daemon` on a **different port**, and the *same* forward must carry a new
//!    connection.
//! 2. **Removing a service cuts the sessions it is carrying** (R22). Only untrusting a
//!    member used to; removing the service changed the stored offer and nothing else.
//! 3. **A backend's reset reaches the far client as a reset** (R22/R23, D11). The splice
...
//! 4. **A refused forward resets the application's connection and says why** (R23). The
...
//!
//! D6's `vox up` half — the SOCKS reply is the host's answer, not a guess made before asking
//! — is proved in `service_rehearsal_proof`, whose untrusted-joiner control used to tolerate
//! the very early "succeeded" this fixes, and was rewritten rather than extended.
//!
//! ## Why it is `#[ignore]`d
//!
