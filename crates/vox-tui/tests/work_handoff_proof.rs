...
fn wait_state(w: &Worker, room: &str, what: &str, r: &str, pred: impl Fn(Option<&serde_json::Value>) -> bool) -> serde_json::Value {
    until(w, None, what, &["room", "board", room, "--json"], |o: &Out| {
        o.ok && pred(resource(&o.json(), r))
    })
    .json()
}

fn held_by<'a>(fp: &'a str, session: &'a str) -> impl Fn(Option<&serde_json::Value>) -> bool + 'a {
    move |r| r.is_some_and(|r| r["state"] == "held" && r["owner_fp"] == fp && r["owner_session"] == session)
}
...
fn a_handoff_moves_ownership_by_fingerprint_and_every_node_agrees() {
    ...
    rt.block_on(async {
        for (w, other, name) in [(alice, bob.fp, "bob-the-builder"), (bob, alice.fp, "boss")] {
            assert!(w
                .node
                .apply(vox_core::node::api::NodeCommand::Trust { fingerprint: other, petname: name.into() })
                .await
                .is_done());
        }
    });
    ...
    assert_eq!(o.code, Some(1), "a second session of the same harness must LOSE, not share: {o:?}");
    assert!(o.stderr.contains("held by") && o.stderr.contains("/b1"), "{o:?}");
    let o = bob.vox(Some("b2"), &["room", "release", r, "own-1"]);
    assert_eq!(o.code, Some(1), "another session's release must have no effect, and say so: {o:?}");
    wait_state(alice, r, "alice to see bob/b1 hold own-1", "own-1", held_by(&b_fp, "b1"));

    // ---- (2) untargeted handoff, completed by a bob session ----
    let o = alice.vox(Some("a1"), &["room", "claim", r, "h-any"]);
    assert!(o.ok, "{o:?}");
    let o = alice.vox(Some("a1"), &["room", "handoff", r, "h-any", "--to", b_prefix]);
    ...
    for w in [alice, bob] {
        ...
        assert!(resource(&b, "h-any").unwrap()["owner_fp"].is_null(), "pending names no owner");
    }
    let o = alice.vox(Some("a2"), &["room", "claim", r, "h-any"]);
    assert_eq!(o.code, Some(1), "a non-recipient must not take a reserved item: {o:?}");
    let o = bob.vox(Some("b2"), &["room", "claim", r, "h-any"]);
    assert!(o.ok && o.stdout.contains("you hold h-any"), "the recipient's claim completes it: {o:?}");
    wait_state(alice, r, "alice to see the handoff completed", "h-any", held_by(&b_fp, "b2"));
    let o = alice.vox(Some("a1"), &["room", "release", r, "h-any"]);
    assert_eq!(o.code, Some(1), "the sender relinquished it; its release must change nothing: {o:?}");

    // ---- (3) a session-targeted handoff ----
    assert!(alice.vox(Some("a1"), &["room", "claim", r, "h-targeted"]).ok);
    let o = alice.vox(Some("a1"), &["room", "handoff", r, "h-targeted", "--to", b_prefix, "--to-session", "b2"]);
    assert!(o.ok, "{o:?}");
    wait_state(bob, r, "bob to see the targeted handoff", "h-targeted", |x| {
        x.is_some_and(|x| x["state"] == "pending" && x["to_session"] == "b2")
    });
    let o = bob.vox(Some("b1"), &["room", "claim", r, "h-targeted"]);
    assert_eq!(o.code, Some(1), "the other session of the recipient harness must not complete it: {o:?}");
    ...
    assert!(alice.vox(Some("a1"), &["room", "handoff", r, "h-decline", "--to", b_prefix]).ok);
    wait_state(bob, r, "bob to see the handoff to decline", "h-decline", |x| x.is_some_and(|x| x["state"] == "pending"));
    let o = bob.vox(Some("b1"), &["room", "decline", r, "h-decline"]);
    assert!(o.ok && o.stdout.contains("it is free"), "{o:?}");
    let b = wait_state(alice, r, "alice to see the decline", "h-decline", |x| x.is_none());
    assert!(resource(&b, "h-decline").is_none(), "a declined item must be free, NOT back with the sender");
    let o = alice.vox(Some("a2"), &["room", "claim", r, "h-decline"]);
    assert!(o.ok && o.stdout.contains("you hold h-decline"), "a fresh claim by a third session: {o:?}");

    // ---- (5) a no-TTL claim's handoff still lapses ----
    assert!(alice.vox(Some("a1"), &["room", "claim", r, "h-expiry"]).ok, "a claim with no --ttl");
    assert!(alice.vox(Some("a1"), &["room", "handoff", r, "h-expiry", "--to", b_prefix, "--ttl", "3"]).ok);
    wait_state(bob, r, "bob to see the short handoff", "h-expiry", |x| x.is_some_and(|x| x["state"] == "pending"));
    std::thread::sleep(std::time::Duration::from_secs(4));
    for w in [alice, bob] {
        let b = board(w, None, r);
        assert!(resource(&b, "h-expiry").is_none(), "{}: the handoff's own deadline must free it: {b}", w.name);
    }
    ...
    until(bob, None, "bob to hold as many entries as alice", &["room", "board", r, "--json"], |o: &Out| {
        o.ok && o.json()["position"]["entries"] == last
    });
    let boards: Vec<serde_json::Value> = [alice, bob].iter().map(|w| agreed(&board(w, None, r))).collect();
    assert_eq!(boards[0], boards[1], "alice and bob fold different boards");
    eprintln!("[proof] identical folded boards on two nodes: {}", boards[0]);
}
