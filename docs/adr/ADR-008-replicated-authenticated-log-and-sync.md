...
- **An answered sync is bound to the room (2026-09-24, PRD-001 D5/R5).** `run_sync_session`
  ...
  **The outbound direction is closed too:** a session this node *starts* pushes a room only to that
  room's members (admitted from the board first, as above) and to this node's own anchors, and an anchor
  forwards a kept room only to its authors. Before, a fresh connection was pushed every open room. Gate:
  the same file, step 4 — the victim's own push, answered by a member of A only: 1 session, 5 of B's
  entries before; 0 and 0 after, with A's 5 still pushed (the control).
