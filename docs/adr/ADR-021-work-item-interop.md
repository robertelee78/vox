...
| F18 | **`adapter_stream_proof` sometimes waits about 120 s longer.** | Measured 2026-09-24 on `feat/prd1-r15-r17` (#14 + D9 + #16 + R15/R17): run durations split into two groups, about 60 s and about 180 s, and one run in eight failed at an `until` whose `TIMEOUT` is 60 s. The extra ~120 s is longer than any single wait, so it is structural, not noise. The cause is not identified. | **Open** (v0.2.10). Remove when the split is explained and the proof's durations are unimodal, or when the delay is shown to be the harness's and fixed there. |
| F19 | **The adapter read cycle assumes local order survives a node restart; that is read from the code, not proved.** | §7's read cycle says the timeline is rebuilt from the sealed plaintext cache in ascending segment order (`channel.rs`, `store.rs`), so an entry hash names a fixed prefix across restarts. No proof restarts a node and compares the order. | **Closed** (v0.2.10): `order_survives_a_restart_proof` restarts alice's `vox daemon` after 42 rows that interleave her appends with rows synced from bob. `read --json` order, `board.position` and `tail --since <pre-restart cursor>` are unchanged. Rebuilding the timeline in hash order turns it red (39 of 42 rows moved), and so does canonical order (30 of 42 moved), which shows the local order it preserves differs from canonical. |
