...
| F18 | **`adapter_stream_proof` sometimes waits about 120 s longer.** | Measured 2026-09-24 on `feat/prd1-r15-r17` (#14 + D9 + #16 + R15/R17): run durations split into two groups, about 60 s and about 180 s, and one run in eight failed at an `until` whose `TIMEOUT` is 60 s. The extra ~120 s is longer than any single wait, so it is structural, not noise. The cause is not identified. | **Closed** (v0.2.10), explained, with no product defect: the slow group is CPU contention. Controlled A/B inside the shared timing lock: quiet 4/4 at 43–48 s, 144 busy loops on 18 cores 4/4 at 113–123 s, all green. Per-phase timing (now printed by the proof as `[phase]`) shows the CPU-bound phases stretching in proportion (room setup 10→33–42 s, burst 11→29 s) while the fixed idle waits stay flat. There is no fixed-size gap, lost event or timeout. No bound was loosened. |
| F19 | **The adapter read cycle assumes local order survives a node restart; that is read from the code, not proved.** | §7's read cycle says the timeline is rebuilt from the sealed plaintext cache in ascending segment order (`channel.rs`, `store.rs`), so an entry hash names a fixed prefix across restarts. No proof restarts a node and compares the order. | **Open** (v0.2.10). Remove when a proof restarts a `vox daemon` mid-room and shows `read --json` order and `board.position` unchanged for the rows it held before the restart. |
| F20 | **The seeded attempt id's two exclusions are unproven.** | §2: a `failed` whose operation is void seeds nothing, and a retried `failed` seeds from its first entry. Both are by reading only; no proof posts a voided or duplicated `failed`. | **Open** (v0.2.10). Remove when `work_ref_proof` (or a sibling) posts both and asserts the seeded id. |
...
- **M21.9 — the drain says when a claim was lost.** Decided 2026-09-24, not built. A lapse, takeover or
  ...
  *Proof*: through the shipped binary, a session whose claim lapses between two drains is told exactly
  ...
  > **Built on PR #14, 2026-09-25** (proof `claim_lost_proof`, two `vox daemon`s and a `vox node` anchor,
  > the real drain hook). The drain
  ...
  > reported. Only the holder can release or hand off, so any other loss is a **lapse**, and the notice
  > adds what the item is now: *now held by `<fp>/<session>`* or *now reserved for …* (the first version
  > called the second "handed off", which review showed could only follow a lapse). Proved: unchanged
  > claims say nothing; a lapse is reported once; a lapsed claim bob re-took names bob; a lapsed claim bob
  > re-took and handed off names the recipient; an own release says nothing **in the same drain that
  > reports another lapse** (the positive control). Five mutants caught on the real-process harness.
- **M21.10 — a `result` warns about unread addressed messages.** Decided 2026-09-24, not built. A
  ...
  *Proof*: through the shipped binary, a `result` posted with an addressed message unread posts and
  ...
  > **Built on PR #14, 2026-09-25** (proof `result_unread_proof`, two `vox daemon`s). `vox room post --type result`
  ...
  > after the drain delivers it, the next result names nothing; a non-`result` post never warns; a message
  > to the session's `VOX_AGENT_NAME` is named. Five mutants caught on the real-process harness (never
  > reports; cursor ignored; addressing ignored; every post warns; `VOX_AGENT_NAME` ignored).
