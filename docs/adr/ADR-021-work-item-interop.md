...
| F20 | **The seeded attempt id's two exclusions are unproven.** | §2: a `failed` whose operation is void seeds nothing, and a retried `failed` seeds from its first entry. Both are by reading only; no proof posts a voided or duplicated `failed`. | **Closed** (v0.2.10): `seeded_attempt_exclusions_proof` posts a retried `failed` (two entries of one operation) and a voided pair (one operation, two contents) through the shipped binary. The next attempt seeds from the retried one's first entry, and the void pair seeds nothing. Each exclusion removed turns it red at its own assertion. |
